...
If more than one service is required for your development environment, you can specify multiple services in the same command with the flag `-s`. For example, to create a dev flow with the frontend and cart services:

```bash
kardinal flow create frontend leoporoli/newobd-frontend:0.0.6 -s productcatalogservice=leoporoli/newobd-productcatalogservice:0.0.3 -s cartservice=leoporoli/newobd-cartservice:0.0.3
```
