...
2. You have distributed tracing in your services

If you can't yet fulfill these requirements, our [demo app](getting-started/demo) is a great way to explore Kardinal until those two prerequisites are met.

<Collapsible title="⚠️ Note: Defining dependencies for all your services">
Right now, Kardinal requires a complete service dependency graph to be defined in your Kubernetes manifests for you to get started. We're working on removing this requirement, but for now, it's a must.
</Collapsible>
...
<Collapsible title="⚠️ Warning: Make sure HTTP ports are marked with appProtocol">
...
To support multiple flows utilizing the same services, Kardinal uses trace ids on requests that determine which versions of services in a cluster to route traffic to. Already have distributed tracing implemented? Great! Currently, we support the following trace ids:
...
If any of these trace ids are used for tracing throughout your application, Kardinal will use these to route requests for varying flows.

Otherwise, you can integrate Kardinals trace id header by having all HTTP applications forward the `X-Kardinal-Trace-Id` header along with the trace id from the incoming request onto all outgoing requests. Applications that are non-http will be duplicated and Kardinal will handle routing to duplicated versions. To read more about trace ids and how the mechanism works read here:
...
Details will differ for different stacks and languages but the high level idea is to forward the `X-Kardinal-Trace-Id` header value from incoming HTTP requests to all internal(to cluster) outgoing HTTP requests! If you have questions on integrating Kardinal tracing onto your stack, please reach out to us.
