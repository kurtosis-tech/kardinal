...
To get started, you just need to add the `kardinal.dev.service/ingress` and `kardinal.dev.service/host` annotations to your Kubernetes manifests.
...
1. *`kardinal.dev.service/ingress`* and *`kardinal.dev.service/host`* to specify the ingress point
2. *`kardinal.dev.service/dependencies`* to specify the dependency graph
3. *`kardinal.dev.service/stateful`* to specify what services hold state
4. *`kardinal.dev.service/plugins`* to specify plugin handlers for external and stateful services
