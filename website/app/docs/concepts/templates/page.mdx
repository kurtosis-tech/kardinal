...
Kardinal Templates provide a powerful way to standardize and simplify the process of creating flows in your Kardinal deployments. They allow DevOps engineers or senior developers to define reusable configurations that can be easily applied by other team members.
...
2. **Stable, Mirrored**: Configures services to share the production environment, with traffic mirrored downstream.
3. **Stable, Redirected**: Similar to mirrored, but with traffic redirected into the service.
4. **Custom**: User-defined templates that can override or extend the base manifest configuration.
...
- **Shared Resources**: Use `kardinal.dev.service/shared: "true"` annotation to specify shared instances of services across flows.
...
This README provides a comprehensive guide to understanding, creating, and using Kardinal Templates.